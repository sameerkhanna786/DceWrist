...
#        y=cy_conv_exp(list(self.time),list(self.curve),list(self.aif),tau)
    ...
    def convert_startdict(self, startdict):
        ...
        Fp = startdict["F"] / 6000.  # we need to convert to SI
...
class ModifiedTofts(CompartmentModel):

    """
    The Modified Tofts model. (

    Taken from S P Sourbron and D L Buckley - tracer kinetic modeliling in MRI
    this needs to be implemented for the whole script.
    T_c = v_p / F_p
    T_e = v_e / PS    mean transit time of EES
    T = (v_p + v_e) / F_p     mean transit time of combined system
    sigma_+ = ((T+T_e) + sqrt(  (T+T_e)**2   -  4*T_c*T_e  ))   /   (2*T_c*T_e)
    ...
    def __init__(self, time=sp.empty(1), curve=sp.empty(1), aif=sp.empty(1), startdict={'vp': 11.2, 'ktrans': 10, 've': 20.}):
        CompartmentModel.__init__(self, time, curve, aif, startdict)
        # override the value of the compartment model:
        self.startdict = startdict
        self.nooffreeparameters = 3
    ...
    def calc_modelfunction(self, parameters):
        ...
        p = parameters
        modelcurve = p[0] * self.intvector() + \
                     p[1] * self.convolution_w_exp(p[2],
    ...
    def convert_startdict(self, startdict):
        ...
        vp = startdict.get("vp") / 100.
        ve = startdict.get("ve") / 100.
        ktrans = startdict.get("ktrans")
        kep = ktrans / ve

        return [vp, ktrans, kep]
    ...
    def get_parameters(self):
        """To be used after a successful fit.
        returns a dictionary with keys vp, ve, ktrans, kep
        """

        vp = self._parameters[0]
        ktrans = self._parameters[1]
        kep = self._parameters[2]
        ve = kep / ktrans

        self.readable_parameters["vp"] = vp * 100.0
        self.readable_parameters["ve"] = ve * 100.0
        self.readable_parameters["ktrans"] = ktrans
        self.readable_parameters["kep"] = kep
